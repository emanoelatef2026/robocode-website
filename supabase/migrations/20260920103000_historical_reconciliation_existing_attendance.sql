-- Historical reconciliation may charge an existing, unlinked attendance row
-- to a new contract. A row already linked to an earlier contract is never moved.

CREATE OR REPLACE FUNCTION public.apply_historical_reconciliation_records(
  p_schedule_ids UUID[], p_funded_count INT, p_student_id UUID,
  p_enrollment_id UUID, p_recorded_by UUID
)
RETURNS TABLE (attendance_record_id UUID, schedule_id UUID, funded BOOLEAN)
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO ''
AS $$
DECLARE
  i INT; len INT; v_record_id UUID; v_already_funded BOOLEAN;
...
BEGIN
  ...
  FOR i IN 1..len LOOP
    ...
    SELECT EXISTS(
      SELECT 1 FROM public.attendance_consumptions
  ...
  IF array_length(v_funded_record_ids, 1) > 0 THEN
    PERFORM public.consume_attendance_sessions_batch(
      v_funded_record_ids, v_funded_enrollment_ids, v_funded_student_ids
    );
  END IF;
