...
function SortableRow({
  ...
  return (
    ...
    >
      ...
      {/* Sort badge */}
      <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#F3F4F6] text-[11px] font-bold text-[#9CA3AF]">
        {index + 1}
      </span>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-[#9CA3AF] font-mono">{section.key}</p>
      </div>
...
export default function HomepageSectionsPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    fetch("/api/studio/homepage-sections")
      .then((r) => r.json())
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch("/api/studio/homepage-sections", {
        method:  "PATCH",
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {saveStatus === "error" && (
            <span className="text-[12px] font-semibold text-[#EF4444]">Save failed — try again</span>
          )}
          <button
            onClick={handleSave}
      ...
      {/* Legend */}
      <div className="flex flex-wrap items-center gap-4 rounded-lg border border-[#F1F5F9] bg-white px-4 py-3 text-[12px] text-[#9CA3AF]">
        <span className="flex items-center gap-1.5">
          <span className="h-3 w-3 rounded-full bg-[#10B981]" /> Visible on homepage
        </span>
        <span className="flex items-center gap-1.5">
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-[#E2E8F0] py-20 text-center">
          <p className="text-[#9CA3AF] text-sm">No sections found.</p>
          <p className="mt-1 text-[#D1D5DB] text-xs">Run the SQL seed in Supabase to populate sections.</p>
        </div>
      ) : (
        <DndContext
          sensors={sensors}
