...
export default function LearningJourneyAdminPage() {
  ...
  const load = () => {
    setLoading(true);
    fetch("/api/studio/learning-journey")
      .then((r) => r.json())
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url    = editingId
      ? `/api/studio/learning-journey/${editingId}`
      : "/api/studio/learning-journey";
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeleting(id);
    await fetch(`/api/studio/learning-journey/${id}`, { method: "DELETE" });
    setDeleting(null);
    load();
  };
  ...
  const handleToggleActive = async (stage: Stage) => {
    ...
    await fetch(`/api/studio/learning-journey/${stage.id}`, { method: "PATCH", body: fd });
    setToggling(null);
    load();
  };
