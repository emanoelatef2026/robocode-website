...
interface Workspace {
  ...
  description: string
  href:        string
  iconKey:     'admin' | 'studio' | 'team-leader'
}
...
const TL_WORKSPACES: Workspace[] = [
  {
    ...
    description: 'Manage students, parents, instructors, attendance, portfolios, certificates, and branch operations.',
...
function WorkspaceIcon({ iconKey }: { iconKey: Workspace['iconKey'] }) {
  if (iconKey === 'admin') {
    return (
        ...
        {workspaces.map((ws) => (
          ...
          >
            <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-[#F8FAFC] text-[#0B1F3A] transition-colors duration-200 group-hover:bg-[#FF8A1F]/10 group-hover:text-[#FF8A1F]">
              <WorkspaceIcon iconKey={ws.iconKey} />
            </div>
