            ...
            <div key={s.student_id} className="flex items-center justify-between gap-3 px-4 py-3">
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center flex-wrap gap-2 mt-0.5">
                  {sessLeft != null && (
                    <span className={`text-[12px] ${isExhausted ? 'text-[#EF4444] font-semibold' : 'text-[#64748B]'}`}>
                      {sessLeft} sess. left
                    </span>
                  )}
                  {s.subscription_amount != null && s.subscription_amount > 0 && (
                    <span className="text-[12px] text-[#64748B]">Pkg: {fmtCurrency(s.subscription_amount)}</span>
                  )}
                  {s.paid_amount > 0 && (
                    <span className="text-[12px] text-[#10B981]">Paid: {fmtCurrency(s.paid_amount)}</span>
                  )}
                  {s.remaining_balance > 0 && (
                    <span className="text-[12px] text-[#EF4444] font-medium">Due: {fmtCurrency(s.remaining_balance)}</span>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                ...
                {isExhausted && (
                  <span className="rounded-full bg-[#FEE2E2] px-2 py-0.5 text-[11px] font-bold text-[#DC2626]">EXHAUSTED</span>
                )}
              </div>
            </div>
          )
