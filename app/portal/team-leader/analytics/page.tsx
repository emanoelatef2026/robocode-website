...
const ALERT_SEVERITY_STYLES: Record<AlertSeverity, { border: string; bg: string; text: string; dot: string }> = {
  critical: { border: 'border-[#FECACA]',    bg: 'bg-[#FEE2E2]',    text: 'text-[#DC2626]',    dot: 'bg-[#EF4444]' },
  warning:  { border: 'border-[#FDE68A]',  bg: 'bg-[#FFFBEB]',  text: 'text-[#B45309]',  dot: 'bg-[#F59E0B]' },
  info:     { border: 'border-blue-200',   bg: 'bg-[#EFF6FF]',   text: 'text-[#1D4ED8]',   dot: 'bg-[#3B82F6]' },
}
    ...
    <div className="space-y-5">
        ...
        <div className="space-y-6">
          ...
          {operationalAlerts && operationalAlerts.length > 0 && (
            <section>
              <h2 className="mb-3 text-base font-semibold text-[#0B1F3A]">
                Action Required
                <span className="ml-2 inline-block rounded-full bg-[#FEE2E2] px-2 py-0.5 text-xs font-semibold text-[#EF4444]">{operationalAlerts.length}</span>
              </h2>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-1.5 md:gap-3 sm:grid-cols-5">
              {[
                { label: 'Active',          value: studentsData.total_active,   color: 'bg-[#38BDF8]' },
                { label: 'New This Month',  value: studentsData.new_this_month, color: 'bg-[#10B981]' },
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-1.5 md:gap-3 sm:grid-cols-4">
              {[
                { label: 'Active Groups',      value: groupsData.total_active,       color: 'bg-[#38BDF8]' },
                { label: 'No Instructor',      value: groupsData.without_instructor, color: groupsData.without_instructor > 0 ? 'bg-[#F59E0B]' : 'bg-[#CBD5E1]' },
                { label: 'No Course',          value: groupsData.without_course,     color: groupsData.without_course > 0 ? 'bg-[#F59E0B]' : 'bg-[#CBD5E1]' },
                { label: 'Low Attendance',     value: groupsData.low_attendance,     color: groupsData.low_attendance > 0 ? 'bg-[#EF4444]' : 'bg-[#CBD5E1]' },
              ].map(k => (
                <div key={k.label} className="min-w-0 ds-card px-2 py-1.5 md:p-3">
                  <div className={`mb-0.5 h-0.5 w-3 rounded-full ${k.color} opacity-80 md:mb-1.5 md:h-1 md:w-6`} />
                  <p className="truncate text-[13px] font-bold leading-none text-[#0B1F3A] md:text-lg">{k.value}</p>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-1.5 md:gap-3 sm:grid-cols-4">
              {[
                { label: 'New This Month',    value: leadsData.new_this_month,      color: 'bg-[#38BDF8]' },
                { label: 'Converted',         value: leadsData.converted_this_month,color: 'bg-[#10B981]' },
                { label: 'Conversion Rate',   value: `${leadsData.conversion_rate_pct}%`, color: leadsData.conversion_rate_pct >= 20 ? 'bg-[#10B981]' : 'bg-[#F59E0B]' },
                { label: 'Overdue Follow-ups',value: leadsData.overdue_followups,   color: leadsData.overdue_followups > 0 ? 'bg-[#EF4444]' : 'bg-[#CBD5E1]' },
              ].map(k => (
                <div key={k.label} className="min-w-0 ds-card px-2 py-1.5 md:p-3">
                  <div className={`mb-0.5 h-0.5 w-3 rounded-full ${k.color} opacity-80 md:mb-1.5 md:h-1 md:w-6`} />
                  <p className="truncate text-[13px] font-bold leading-none text-[#0B1F3A] md:text-lg">{k.value}</p>
                    ...
                    {branchComparison.map(b => (
                      <tr key={b.branch_id} className="border-b border-[#E2E8F0] last:border-0">
                        ...
                        <td className="px-4 py-2.5">
                          {b.at_risk_count > 0
                            ? <span className="inline-block rounded-full bg-[#FEE2E2] px-2 py-0.5 text-xs font-medium text-[#EF4444]">{b.at_risk_count}</span>
      ...
      {tab === 'students' && studentsData && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
            {[
              { label: 'Active Students',   value: studentsData.total_active,   color: 'bg-[#38BDF8]' },
              { label: 'New This Month',    value: studentsData.new_this_month, color: 'bg-[#10B981]' },
              { label: 'Inactive',          value: studentsData.inactive,        color: 'bg-[#CBD5E1]' },
              { label: 'Monthly Attendance',value: `${studentsData.attendance_avg}%`, color: studentsData.attendance_avg >= 75 ? 'bg-[#10B981]' : 'bg-[#F59E0B]' },
              { label: 'At Risk',           value: studentsData.at_risk_count,  color: studentsData.at_risk_count > 0 ? 'bg-[#EF4444]' : 'bg-[#CBD5E1]' },
            ].map(k => (
              <div key={k.label} className="min-w-0 ds-card px-2 py-1.5 md:p-3">
                <div className={`mb-0.5 h-0.5 w-3 rounded-full ${k.color} opacity-80 md:mb-1.5 md:h-1 md:w-6`} />
          ...
          {atRiskResult && atRiskResult.data.length === 0 && (
            <div className="ds-card px-6 py-10 text-center">
              <p className="text-sm text-[#10B981] font-medium">No at-risk students.</p>
        ...
        <div className="space-y-6">
          ...
          <div className="ds-card p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className={`text-2xl font-bold ${financeKPIs.collection_rate_pct >= 80 ? 'text-[#10B981]' : financeKPIs.collection_rate_pct >= 60 ? 'text-[#F59E0B]' : 'text-[#EF4444]'}`}>
                {financeKPIs.collection_rate_pct}%
              </span>
            </div>
            <div className="h-2 w-full rounded-full bg-[#F1F5F9] overflow-hidden">
              <div
                className={`h-full rounded-full transition-all ${financeKPIs.collection_rate_pct >= 80 ? 'bg-[#10B981]' : financeKPIs.collection_rate_pct >= 60 ? 'bg-[#F59E0B]' : 'bg-[#EF4444]'}`}
          ...
          {collectionQueue && (collectionQueue.overdue30.length > 0 || collectionQueue.overdue14.length > 0 || collectionQueue.dueThisWeek.length > 0) && (
            <section>
              ...
              {collectionQueue.overdue30.length > 0 && (
                <div className="mb-3">
                  <p className="mb-2 text-xs font-medium text-[#EF4444]">Overdue 30+ days ({collectionQueue.overdue30.length})</p>
                  <div className="rounded-xl border border-[#FEE2E2] bg-[#FEE2E2] overflow-x-auto">
                    <table className="w-full text-sm">
                      <tbody>
                        {collectionQueue.overdue30.slice(0, 5).map(item => (
                          <tr key={item.account_id} className="border-b border-[#FEE2E2] last:border-0">
                            ...
                            <td className="px-4 py-2 text-right font-medium text-[#EF4444]">EGP {fmt(item.remaining_amount)}</td>
              ...
              {collectionQueue.dueThisWeek.length > 0 && (
                <div>
                  <p className="mb-2 text-xs font-medium text-[#F59E0B]">Due This Week ({collectionQueue.dueThisWeek.length})</p>
                  <div className="rounded-xl border border-amber-100 bg-[#FFFBEB] overflow-x-auto">
                    <table className="w-full text-sm">
                      <tbody>
                        {collectionQueue.dueThisWeek.slice(0, 5).map(item => (
                          <tr key={item.account_id} className="border-b border-amber-100 last:border-0">
                            <td className="px-4 py-2 font-medium text-[#0B1F3A]">{item.student_name}</td>
                            <td className="px-4 py-2 text-[#64748B] text-xs">{item.group_name ?? '—'}</td>
                            <td className="px-4 py-2 text-right font-medium text-[#B45309]">EGP {fmt(item.remaining_amount)}</td>
          ...
          {brokenPromises && brokenPromises.length > 0 && (
            <section>
              <h2 className="mb-3 text-base font-semibold text-[#0B1F3A]">
                ...
                <span className="ml-2 inline-block rounded-full bg-[#FEE2E2] px-2 py-0.5 text-xs font-semibold text-[#EF4444]">{brokenPromises.length}</span>
              </h2>
              <div className="rounded-xl border border-[#FEE2E2] bg-white overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="ds-table-head">
                    <tr className="border-b border-[#E2E8F0] bg-[#F8FAFC]">
                  ...
                  <tbody>
                    {brokenPromises.slice(0, 8).map(p => (
                      <tr key={p.id} className="ds-table-row">
                        ...
                        <td className="px-4 py-2.5 text-right text-xs text-[#EF4444]">{p.promised_date}</td>
                        <td className="px-4 py-2.5 text-right font-medium text-[#EF4444]">EGP {fmt(p.remaining_amount ?? 0)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              { label: 'Active Groups',    value: groupsData.total_active,       color: 'bg-[#38BDF8]' },
              { label: 'No Instructor',    value: groupsData.without_instructor, color: groupsData.without_instructor > 0 ? 'bg-[#F59E0B]' : 'bg-[#CBD5E1]' },
              { label: 'No Active Course', value: groupsData.without_course,     color: groupsData.without_course > 0 ? 'bg-[#F59E0B]' : 'bg-[#CBD5E1]' },
              { label: 'Low Attendance',   value: groupsData.low_attendance,     color: groupsData.low_attendance > 0 ? 'bg-[#EF4444]' : 'bg-[#CBD5E1]' },
            ].map(k => (
              <div key={k.label} className="min-w-0 ds-card px-2 py-1.5 md:p-3">
                <div className={`mb-0.5 h-0.5 w-3 rounded-full ${k.color} opacity-80 md:mb-1.5 md:h-1 md:w-6`} />
                ...
                <tbody>
                  {groupsData.groups.map(g => (
                    <tr key={g.group_id} className="ds-table-row">
                      ...
                      <td className="px-4 py-2.5">
                        {g.has_instructor
                          ? <span className="inline-block rounded-full bg-[#E7F8EE] px-2 py-0.5 text-xs font-medium text-[#15803D]">Yes</span>
                          : <span className="inline-block rounded-full bg-[#FFFBEB] px-2 py-0.5 text-xs font-medium text-[#B45309]">Missing</span>}
                      </td>
                      <td className="px-4 py-2.5">
                        {g.has_active_course
                          ? <span className="inline-block rounded-full bg-[#E7F8EE] px-2 py-0.5 text-xs font-medium text-[#15803D]">Yes</span>
                    ...
                    <tr key={i.instructor_id} className="ds-table-row">
                      ...
                      <td className="px-4 py-2.5">
                        {i.avg_student_rating !== null
                          ? <span className={`font-semibold ${i.avg_student_rating >= 4 ? 'text-[#10B981]' : i.avg_student_rating >= 3 ? 'text-[#F59E0B]' : 'text-[#EF4444]'}`}>
      ...
      {tab === 'leads' && leadsData && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              { label: 'Total Leads',        value: leadsData.total,               color: 'bg-[#38BDF8]' },
              { label: 'New This Month',     value: leadsData.new_this_month,      color: 'bg-[#10B981]' },
              { label: 'Converted',          value: leadsData.converted_this_month,color: 'bg-[#10B981]' },
              { label: 'Overdue Follow-ups', value: leadsData.overdue_followups,   color: leadsData.overdue_followups > 0 ? 'bg-[#EF4444]' : 'bg-[#CBD5E1]' },
            ].map(k => (
              <div key={k.label} className="min-w-0 ds-card px-2 py-1.5 md:p-3">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div className="ds-card p-5">
              ...
              <p className={`text-xl font-bold md:text-3xl ${leadsData.conversion_rate_pct >= 20 ? 'text-[#10B981]' : 'text-[#F59E0B]'}`}>
      ...
      {tab === 'assignments' && assignmentData && (
        <div className="space-y-5">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
            {[
              { label: 'Published',       value: assignmentData.kpis.total_published,      color: 'bg-[#38BDF8]' },
              { label: 'Overdue',         value: assignmentData.kpis.overdue_count,         color: assignmentData.kpis.overdue_count > 0 ? 'bg-[#F59E0B]' : 'bg-[#CBD5E1]' },
              { label: 'Avg Completion',  value: `${assignmentData.kpis.avg_completion_pct}%`, color: assignmentData.kpis.avg_completion_pct >= 70 ? 'bg-[#10B981]' : 'bg-[#EF4444]' },
              { label: 'Pending Review',  value: assignmentData.kpis.pending_review_count, color: assignmentData.kpis.pending_review_count > 0 ? 'bg-[#F59E0B]' : 'bg-[#CBD5E1]' },
              { label: 'Delayed 3d+',    value: assignmentData.kpis.grading_delay_count,  color: assignmentData.kpis.grading_delay_count > 0 ? 'bg-[#EF4444]' : 'bg-[#CBD5E1]' },
            ].map(k => (
              <div key={k.label} className="min-w-0 ds-card px-2 py-1.5 md:p-3">
                <div className={`mb-0.5 h-0.5 w-3 rounded-full ${k.color} opacity-80 md:mb-1.5 md:h-1 md:w-6`} />
                <p className="truncate text-[13px] font-bold leading-none text-[#0B1F3A] md:text-lg">{k.value}</p>
                        ...
                        <td className="px-4 py-2.5">
                          <div className="flex items-center gap-2">
                            <div className="h-1.5 w-16 rounded-full bg-[#F1F5F9]">
                              <div className={`h-full rounded-full ${i.avg_completion_pct >= 70 ? 'bg-[#10B981]' : i.avg_completion_pct >= 40 ? 'bg-[#F59E0B]' : 'bg-[#EF4444]'}`} style={{ width: `${i.avg_completion_pct}%` }} />
                            </div>
                    ...
                    {assignmentData.by_group.map(g => (
                      <tr key={g.group_id} className="ds-table-row">
                        ...
                        <td className="px-4 py-2.5">
                          <div className="flex items-center gap-2">
                            <div className="h-1.5 w-16 rounded-full bg-[#F1F5F9]">
                              <div className={`h-full rounded-full ${g.completion_pct >= 70 ? 'bg-[#10B981]' : g.completion_pct >= 40 ? 'bg-[#F59E0B]' : 'bg-[#EF4444]'}`} style={{ width: `${g.completion_pct}%` }} />
                            </div>
