    ...
    <div className="fixed inset-0 z-[70] flex flex-col justify-end bg-black/50 md:items-center md:justify-center md:p-4">
      <div className="w-full flex flex-col bg-white shadow-2xl overflow-hidden rounded-t-2xl max-h-[90dvh] md:rounded-2xl md:max-h-none md:max-w-lg">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4">
            ...
            <div className="flex gap-2">
              {(['lead', 'assistant'] as const).map(r => (
                <button key={r} type="button" onClick={() => setRole(r)}
                  className={`flex-1 rounded-lg border py-2 text-[13px] font-medium capitalize transition ${role === r ? 'border-[#0E7490] bg-[#FFF7ED] text-[#C2410C]' : 'border-[#E2E8F0] text-[#64748B] hover:border-[#0E7490]'}`}>
                  {r}
                </button>
              ))}
            </div>
          ...
          <div>
            <label className="mb-1.5 block text-[13px] font-medium text-[#374151]">Search Groups</label>
          ...
          {selectedGroup && (
            remaining <= 0 ? (
              <div className="rounded-lg border border-[#FDE68A] bg-[#FFFBEB] px-3 py-2 text-[13px] text-[#B45309]">
              ...
              <div className="flex gap-3">
                <div className="flex-1">
                  <label className="mb-1.5 block text-[13px] font-medium text-[#374151]">From Session</label>
                  <div className="flex h-9 items-center rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] px-3 text-[13px] text-[#64748B]">
                    {fromSession}
                    <span className="ml-1 text-[12px] text-[#94A3B8]">of {selectedGroup.total_sessions ?? '∞'}</span>
                  </div>
                  <p className="mt-0.5 text-[11px] text-[#94A3B8]">Computed from existing allocations</p>
                </div>
                <div className="flex-1">
                  <label className="mb-1.5 block text-[13px] font-medium text-[#374151]">
                    Sessions to Teach
                    <span className="ml-1 text-[12px] font-normal text-[#94A3B8]">({remaining} left)</span>
                  </label>
                  <input
                    type="number"
                    min={1}
                    ...
                    className="w-full rounded-lg border border-[#E2E8F0] px-3 py-2 text-[13px] outline-none focus:border-[#0E7490]"
                  />
