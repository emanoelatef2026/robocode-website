        ...
        <div className="border-b border-[#FEE2E2] bg-[#FEE2E2] px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[13px] text-[#EF4444]">This action cannot be undone</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-5 space-y-4">
          <div className="rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-4 space-y-2">
            <div className="flex items-center justify-between">
              ...
              {instructorCode && (
                <span className="rounded bg-[#F1F5F9] px-1.5 py-0.5 font-mono text-[11px] text-[#94A3B8]">{instructorCode}</span>
              )}
            </div>
          ...
          {isBlocked ? (
            <div className="rounded-xl border border-[#FDE68A] bg-[#FFFBEB] p-4 space-y-1.5">
              <p className="text-[13px] font-semibold text-[#92400E]">Cannot delete — active obligations remain:</p>
              {activeGroupCount > 0 && (
                <p className="text-[13px] text-[#B45309]">• {activeGroupCount} active group(s) still assigned</p>
              )}
              {futureSessionCount > 0 && (
                <p className="text-[13px] text-[#B45309]">• {futureSessionCount} upcoming session(s) scheduled</p>
              )}
              <p className="mt-1 text-[12px] text-[#F59E0B]">Remove from active groups and cancel upcoming sessions first, or archive to preserve the record.</p>
            </div>
          ) : (
            <>
