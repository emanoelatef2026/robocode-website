...
import { reviewPortfolioProject, assignProjectBadge } from '@/modules/portfolio/instructor-actions'
  ...
  return (
    <div className="space-y-4">
      ...
      <form action={reviewAction} className="space-y-3">
        ...
        {reviewState?.success && (
          <div className="rounded-lg bg-[#E7F8EE] px-3 py-2 text-xs text-[#15803D]">Review saved.</div>
        )}
        ...
        <div>
          ...
          <textarea name="instructor_feedback" rows={2} defaultValue={currentFeedback ?? ''}
            placeholder="What did they do well? What needs improvement?" className={cls} />
        </div>

        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="mb-1 block text-xs font-medium text-[#64748B]">Score (0–100)</label>
            <input name="final_score" type="number" min={0} max={100} defaultValue={currentScore ?? ''} placeholder="e.g. 85" className={cls} />
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-[#64748B]">Decision</label>
            <select name="status" defaultValue="approved" className={cls}>
        ...
        <div className="flex items-center gap-3">
          ...
          <button type="button" onClick={() => setShowBadge((b) => !b)}
            className="rounded-lg border border-[#E2E8F0] px-4 py-2 text-sm font-medium text-[#64748B] hover:border-[#C2410C] transition">
      ...
      {showBadge && (
        <form action={badgeAction} className="space-y-3 rounded-lg border border-[#E2E8F0] p-3">
          ...
          <div>
            ...
            <input name="description" placeholder="e.g. Exceptional creativity" className={cls} />
          </div>
