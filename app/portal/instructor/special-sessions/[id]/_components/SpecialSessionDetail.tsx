...
export default function SpecialSessionDetail(props: Props) {
  ...
  const badgeColor = isTrial ? 'bg-[#E0F7FA] text-[#0E7490]' : 'bg-orange-100 text-orange-700'
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className={`rounded-full px-2 py-0.5 text-[11px] font-bold uppercase ${badgeColor}`}>
              {badgeLabel}
            </span>
      ...
      {session.notes && (
        <div className="rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] p-3">
          <p className="text-[13px] font-semibold text-[#64748B] mb-1">Notes</p>
...
function TrialStudentsPanel({
  ...
  return (
    <div className="ds-card overflow-hidden">
        ...
        {isEditable && addMode === 'none' && (
          <div className="flex gap-2">
            ...
            <button onClick={() => setAddMode('new')}
              className="rounded-lg bg-[#0E7490] px-3 py-1.5 text-[13px] font-semibold text-white hover:bg-[#155E75]">
              + New Student
            </button>
          </div>
        )}
        {addMode !== 'none' && (
          <button onClick={() => setAddMode('none')} className="text-[13px] text-[#64748B] hover:underline">
            Cancel
          </button>
        )}
      </div>
      ...
      {/* Add from lead */}
      {addMode === 'lead' && (
        <form action={fromLeadAction} className="border-b border-[#E2E8F0] p-4 space-y-3 bg-[#F0FAFB]">
          {!fromLeadState.success && fromLeadState.error?.message && (
            <p className="text-[13px] text-red-600">{fromLeadState.error.message}</p>
          )}
          ...
          <div>
            ...
            {leads.length === 0 ? (
              <p className="text-[13px] text-[#94A3B8]">No available leads for this branch.</p>
            ) : (
          ...
          <button type="submit" disabled={fromLeadPending || leads.length === 0}
            className="rounded-lg bg-[#0E7490] px-4 py-2 text-[13px] font-semibold text-white disabled:opacity-60">
      ...
      {addMode === 'new' && (
        <form action={newAction} className="border-b border-[#E2E8F0] p-4 space-y-3 bg-[#F0FAFB]">
          {!newState.success && newState.error?.message && (
            <p className="text-[13px] text-red-600">{newState.error.message}</p>
          )}
          <input type="hidden" name="schedule_id" value={session.id} />
            ...
            <div key={s.id}>
              ...
              <div className="flex items-center justify-between gap-3 px-4 py-3">
                <div className="min-w-0 flex-1">
                  ...
                  {s.notes && (
                    <p className="text-[12px] text-[#94A3B8] mt-0.5 italic truncate">{s.notes}</p>
                  )}
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <select
                    ...
                    disabled={!isOngoing}
                    className="rounded-lg border border-[#E2E8F0] px-2 py-1.5 text-[13px] focus:outline-none focus:ring-2 focus:ring-[#0E7490] disabled:opacity-60 disabled:bg-[#F8FAFC]"
                  >
                  ...
                  {isEditable && (
                    <button
                      ...
                      className={`rounded-lg border px-2.5 py-1.5 text-[12px] font-semibold transition ${
                        editingId === s.id
                          ? 'border-[#0E7490] bg-[#F0FAFB] text-[#0E7490]'
                          : 'border-[#E2E8F0] text-[#64748B] hover:border-[#0E7490] hover:text-[#0E7490]'
                      }`}
                    >
      ...
      {isOngoing && session.students.length > 0 && (
        <div className="border-t border-[#E2E8F0] px-4 py-3 flex items-center gap-3">
          <button
            ...
            className="rounded-lg bg-[#0E7490] px-4 py-2 text-[13px] font-semibold text-white disabled:opacity-60 hover:bg-[#155E75]"
          >
  ...
  return (
    <form action={formAction} className="border-t border-dashed border-[#E2E8F0] bg-[#FAFBFF] px-4 py-3 space-y-3">
      {!state.success && state.error?.message && (
        <p className="text-[13px] text-red-600">{state.error.message}</p>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            required
            className="w-full rounded-lg border border-[#E2E8F0] px-3 py-1.5 text-[13px] focus:outline-none focus:ring-2 focus:ring-[#0E7490]"
          />
        </div>
        <div>
          <label className="block text-[12px] font-semibold text-[#64748B] mb-1">Parent Phone</label>
      ...
      <button
        type="submit"
        disabled={pending}
        className="rounded-lg bg-[#0B1F3A] px-4 py-1.5 text-[13px] font-semibold text-white disabled:opacity-60 hover:bg-[#1E3A5F]"
      >
  ...
  return (
    <div className="ds-card overflow-hidden">
      <div className="border-b border-[#E2E8F0] px-4 py-3 flex items-center justify-between">
        ...
        <span className="text-[13px] text-[#64748B]">{session.students.length} registered</span>
      </div>
            ...
            {session.students.map(s => (
              <div key={s.id} className="flex items-center justify-between px-4 py-3 gap-3">
                ...
                <select
                  ...
                  disabled={!isOngoing}
                  className="rounded-lg border border-[#E2E8F0] px-2 py-1.5 text-[13px] focus:outline-none focus:ring-2 focus:ring-[#F59E0B] disabled:opacity-60"
                >
                  <option value="present">Present</option>
                  <option value="absent">Absent</option>
