    ...
    <div className="space-y-3 md:space-y-4">
        ...
        {TABS.map(t => (
          <Link
            ...
            className={`px-3.5 py-2 text-[13px] font-semibold transition ${
              tab === t.key
                ? 'border-b-2 border-[#C2410C] text-[#0B1F3A]'
      ...
      {sessions.length === 0 ? (
        ...
        <div className="overflow-hidden ds-card">
          ...
          <div className="hidden lg:grid grid-cols-[110px_1fr_1fr_90px_1fr_130px_100px] gap-3 border-b border-[#F1F5F9] bg-[#F8FAFC] px-4 py-2 text-[11px] font-bold uppercase tracking-widest text-[#94A3B8]">
            <span>Date</span>
            <span>Group</span>
            <span>Course</span>
            <span>Session</span>
          ...
          <div className="divide-y divide-[#F1F5F9]">
            {sessions.map(s => {
              ...
              const typeBadge = s.session_type === 'trial'
                ? { label: '🟣 Trial',  style: 'bg-[#E0F7FA] text-[#0E7490]' }
              ...
              return (
                ...
                >
                  ...
                  <div className="px-4 py-3 lg:hidden">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className="text-[12px] text-[#94A3B8] shrink-0">{dateShort}</span>
                        {typeBadge ? (
                          <span className={`rounded-full px-1.5 py-0.5 text-[11px] font-bold shrink-0 ${typeBadge.style}`}>
                            {typeBadge.label}
                          </span>
                        ) : (
                          <span className="text-[12px] font-bold text-[#0B1F3A] shrink-0">
                    ...
                    <div className="mt-1 flex items-baseline gap-1.5 min-w-0">
                      ...
                      {s.course_title && (
                        <p className="text-[12px] text-[#64748B] truncate shrink-0 leading-tight">{s.course_title}</p>
                      )}
                    </div>
                    <div className="mt-1 flex items-center justify-between gap-2">
                      <p className={`text-[12px] truncate ${s.topic ? 'text-[#64748B]' : 'italic text-[#CBD5E1]'}`}>
                        {s.topic ?? 'No topic'}
                      </p>
                      {s.total_students > 0 && (
                  ...
                  <div className="hidden lg:grid grid-cols-[110px_1fr_1fr_90px_1fr_130px_100px] items-center gap-3 px-4 py-3">
                    ...
                    <span className="text-[13px] text-[#64748B] truncate">{s.course_title || '—'}</span>
                    {typeBadge ? (
                      <span className={`inline-flex w-fit rounded-full px-2 py-0.5 text-[11px] font-bold ${typeBadge.style}`}>
                        {typeBadge.label}
                      </span>
                    ) : (
                    ...
                    <span className={`text-[13px] truncate ${s.topic ? 'text-[#64748B]' : 'italic text-[#CBD5E1]'}`}>
                      {s.topic ?? 'No topic'}
                    </span>
                    {s.total_students > 0 ? (
