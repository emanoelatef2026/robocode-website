...
export default async function ParentChildCertificatesPage({ params }: Props) {
  const user       = await requirePortalRole('parent')
  const { studentId } = await params

  const certificates = await getChildCertificates(user.id, studentId)

  // If the array is empty and the student link doesn't exist, getChildCertificates
  // returns [] rather than throwing. Treat as not-found only if we can't verify linkage.
  // (Linkage is verified inside getChildCertificates — empty means either no certs or no link.)

  return (
    <div className="mx-auto max-w-3xl px-4 py-8 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#0B1F3A]">Certificates</h1>
