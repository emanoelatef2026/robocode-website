    ...
    <>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={openAdd}
            className="flex items-center gap-2 rounded-lg bg-[#0B132B] px-4 py-2.5 text-[13px] font-semibold text-white transition hover:bg-[#19C6F4]"
          >
          ...
          {projects.map((p) => (
            <div key={p.id} className="group rounded-xl border border-gray-100 bg-white shadow-sm overflow-hidden">
              {p.image_url ? (
                ...
                <div className="flex h-40 items-center justify-center bg-gradient-to-br from-[#0B132B] to-[#19C6F4]/30">
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-2">
                  <h3 className="text-[14px] font-semibold text-[#0B132B] leading-snug">{p.title}</h3>
                  {p.featured && (
                    <span className="shrink-0 rounded-full bg-[#19C6F4]/10 px-2 py-0.5 text-[10px] font-bold text-[#19C6F4]">Featured</span>
                  )}
                </div>
                {p.description && (
                ...
                <div className="mt-3 flex gap-2">
                  <button
                    onClick={() => openEdit(p)}
                    className="flex-1 rounded-lg border border-gray-200 py-1.5 text-[12px] font-medium text-gray-500 transition hover:border-[#19C6F4] hover:text-[#19C6F4]"
                  >
                    Edit
                  </button>
                  <button
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 px-4 py-10">
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-xl">
            ...
            <div className="space-y-4 px-6 py-5">
              <Field label="Title *">
                <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className={INPUT} placeholder="e.g. AI Robot Assistant" />
              </Field>
              <Field label="Description">
                <textarea value={form.description ?? ""} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={3} className={INPUT} placeholder="Short project description…" />
              </Field>
              <Field label="Technologies (comma-separated)">
                <input value={(form.technologies ?? []).join(", ")} onChange={(e) => setForm({ ...form, technologies: e.target.value.split(",").map((t) => t.trim()).filter(Boolean) })} className={INPUT} placeholder="Python, Raspberry Pi, OpenCV" />
              </Field>
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Age Group">
                  <input value={form.age_group ?? ""} onChange={(e) => setForm({ ...form, age_group: e.target.value })} className={INPUT} placeholder="8-12 yrs" />
                </Field>
              </div>
              <Field label="Video URL">
                <input value={form.video_url ?? ""} onChange={(e) => setForm({ ...form, video_url: e.target.value })} className={INPUT} placeholder="https://youtube.com/…" />
              </Field>
              ...
              <label className="flex items-center gap-2 text-[13px] text-gray-600">
                <input type="checkbox" checked={form.featured} onChange={(e) => setForm({ ...form, featured: e.target.checked })} className="h-4 w-4 accent-[#19C6F4]" />
                Mark as Featured
              </label>
            </div>

            <div className="flex justify-end gap-3 border-t border-gray-100 px-6 py-4">
              <button onClick={() => setModal(null)} className="rounded-lg px-4 py-2 text-[13px] text-gray-500 hover:bg-gray-100">Cancel</button>
              <button
                onClick={handleSave}
                disabled={saving || !form.title.trim()}
                className="rounded-lg bg-[#0B132B] px-5 py-2 text-[13px] font-semibold text-white transition hover:bg-[#19C6F4] disabled:opacity-50"
              >
...
const INPUT = "w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5 text-[13px] text-[#0B132B] outline-none transition focus:border-[#19C6F4] focus:ring-2 focus:ring-[#19C6F4]/20";
