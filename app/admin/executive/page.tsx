...
function HealthBadge({ score }: { score: number }) {
  const { label, cls } =
    score >= 85 ? { label: 'Excellent', cls: 'bg-[#E7F8EE] text-[#15803D]' } :
    score >= 70 ? { label: 'Good',      cls: 'bg-[#EFF6FF] text-[#1D4ED8]'     } :
...
function HealthBar({ score }: { score: number }) {
  const color = score >= 85 ? 'bg-[#10B981]' : score >= 70 ? 'bg-[#38BDF8]' : score >= 55 ? 'bg-[#F59E0B]' : 'bg-[#EF4444]'
  return (
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-8">
        {[
          ...
          { label: 'Revenue Collected', value: `EGP ${fmt(totalRevenue)}`,        cls: 'text-[#10B981]', badge: null },
          { label: 'Outstanding',       value: `EGP ${fmt(totalOutstanding)}`,    cls: 'text-[#F59E0B]',   badge: null },
          { label: 'Overdue Accounts',  value: String(totalOverdue),              cls: totalOverdue > 0 ? 'text-[#EF4444]' : 'text-[#10B981]', badge: totalOverdue > 0 ? 'alert' : null },
          { label: 'At-Risk Students',  value: String(totalRisk),                 cls: totalRisk > 0 ? 'text-[#EF4444]' : 'text-[#10B981]',   badge: totalRisk > 0 ? 'alert' : null },
          { label: 'Open Tasks',        value: String(totalTasks),                cls: totalTasks > 0 ? 'text-[#F59E0B]' : 'text-[#10B981]', badge: null },
          { label: 'Avg Collection',    value: `${avgCollection}%`,               cls: avgCollection >= 80 ? 'text-[#10B981]' : 'text-[#F59E0B]', badge: null },
          { label: 'Avg Health Score',  value: String(avgHealth),                 cls: avgHealth >= 80 ? 'text-[#10B981]' : avgHealth >= 60 ? 'text-[#F59E0B]' : 'text-[#EF4444]', badge: null },
        ].map(k => (
      ...
      {(criticalIssues > 0 || highIssues > 0) && (
        <div className="rounded-xl border border-[#FECACA] bg-[#FEE2E2] p-4">
          <p className="text-sm font-semibold text-[#991B1B]">Data Integrity Issues Detected</p>
      ...
      <div>
        ...
        {branches.length === 0 ? (
          <div className="ds-card px-6 py-10 text-center text-sm text-[#94A3B8]">
          ...
          <div className="space-y-3">
            {[...branches].sort((a, b) => b.operational_health_score - a.operational_health_score).map((b, idx) => (
              <div key={b.branch_id} className="ds-card p-4">
                ...
                <div className="mt-3 grid grid-cols-3 gap-3 sm:grid-cols-6 text-center text-[11px]">
                  {[
                    ...
                    { label: 'Outstanding',value: `EGP ${fmt(b.outstanding)}`, cls: b.outstanding > 0 ? 'text-[#F59E0B]' : 'text-[#10B981]' },
                    { label: 'Collection', value: `${b.collection_rate_pct}%`, cls: b.collection_rate_pct >= 80 ? 'text-[#10B981]' : 'text-[#F59E0B]' },
                    { label: 'At Risk',    value: String(b.risk_students),   cls: b.risk_students > 0 ? 'text-[#EF4444]' : 'text-[#10B981]' },
                    { label: 'Open Tasks', value: String(b.open_tasks),       cls: b.open_tasks > 5 ? 'text-[#F59E0B]' : 'text-[#0B1F3A]' },
                  ].map(k => (
              ...
              <tbody>
                {(retentionRows as any[]).map(r => (
                  <tr key={r.course_id} className="border-b border-[#F1F5F9] last:border-0 hover:bg-[#F8FAFC]">
                    ...
                    <td className="px-4 py-3 font-medium text-[#10B981]">EGP {fmt(Number(r.total_contracted ?? 0))}</td>
                    <td className="px-4 py-3">
                      {r.retention_pct != null ? (
                        <span className={`font-semibold ${Number(r.retention_pct) >= 70 ? 'text-[#10B981]' : Number(r.retention_pct) >= 50 ? 'text-[#F59E0B]' : 'text-[#EF4444]'}`}>
                          {r.retention_pct}%
                        </span>
                      ) : <span className="text-[#94A3B8]">—</span>}
                    </td>
                    <td className="px-4 py-3">
                      <span className={`font-semibold ${Number(r.dropout_pct) > 20 ? 'text-[#EF4444]' : 'text-[#64748B]'}`}>
                        {r.dropout_pct}%
                      </span>
                    </td>
                    <td className="px-4 py-3">
