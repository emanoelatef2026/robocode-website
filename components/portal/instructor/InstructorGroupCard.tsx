...
export default function InstructorGroupCard({ g }: { g: InstructorGroupCardData }) {
  ...
  return (
    <Link
      ...
      className="ds-card group flex gap-4 p-4 md:p-5 transition-all duration-150 hover:border-[#B8C6D6] hover:shadow-[0_4px_16px_rgba(11,31,58,.10)] active:bg-[#F8FAFC]"
    >
      {/* Icon */}
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#E0F7FA]">
        <svg viewBox="0 0 20 20" fill="#0E7490" className="h-5 w-5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <p className="truncate font-semibold text-[#0B1F3A] transition-colors group-hover:text-[#0E7490]">
              {g.group_name}
            </p>
            {g.group_code && (
              <p className="text-[11px] text-[#64748B]">{g.group_code}</p>
            )}
          </div>
        ...
        {g.branch_name && (
          <p className="mt-0.5 truncate text-[11px] text-[#64748B]">{g.branch_name}</p>
        )}
        ...
        {isActive && myTotal != null && myTotal > 0 ? (
          <div className="mt-3">
            ...
            <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-[#F1F5F9]">
              <div
                className="h-full rounded-full bg-[#C2410C] transition-[width] duration-300"
          ...
          <p className="mt-2 text-xs text-[#64748B]">No sessions yet</p>
        ) : null}
