...
export async function getPayrollItemsForRun(runId: string): Promise<PayrollItemRow[]> {
  ...
  return ((data ?? []) as any[]).map(row => ({
    ...row,
    instructor_name:   instructorName(row.instructors),
...
export async function getPayrollRunKPIs(runId: string): Promise<PayrollKPIs> {
  ...
  const [runRes, itemsRes] = await Promise.all([
    ...
    db.from('payroll_items').select('gross_amount, final_amount, sessions_count, status, currency').eq('payroll_run_id', runId),
  ])
