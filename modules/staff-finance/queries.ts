...
// Does NOT depend on instructor_branches being populated.
...
export async function getLiveInstructorFinance(
  ...
  // 1. Completed schedules in date range for these branches (flat — no nested joins)
  ...
  // 2. Resolve group_course_id → group_id + instructor_id (separate query, no join)
  ...
  for (const gc of gcMap.values()) {
    if (!gc.instructor_id) missingGroupIds.add(gc.group_id)
  }
  ...
  if (missingGroupIds.size > 0) {
    const { data: giData } = await db
      ...
      .select('group_id, instructor_id, role')
  ...
  // 4. Count sessions per instructor
  type SessionEntry = {
    sessions_by_group: Map<string, number>
  ...
  for (const s of schedules) {
    ...
    if (!sessionMap.has(instructorId)) {
      sessionMap.set(instructorId, { sessions_by_group: new Map(), group_ids: new Set(), branch_id: s.branch_id })
    }
    const entry = sessionMap.get(instructorId)!
    entry.group_ids.add(gc.group_id)
    entry.sessions_by_group.set(gc.group_id, (entry.sessions_by_group.get(gc.group_id) ?? 0) + 1)
  }
  ...
  // 7. Load branches for display names
  ...
  for (const b of (branchData ?? []) as any[]) {
    branchNameMap.set(b.id, b.name)
  }

  // 6. Load adjustments for these instructors in date range
  ...
  return instructorIds.map(instructorId => {
    ...
    const sessions_count   = [...entry.sessions_by_group.values()].reduce((a, b) => a + b, 0)
    const group_count      = entry.group_ids.size
    const salary           = Number(inst?.salary_per_session ?? 0)
    const session_earnings = sessions_count * salary
...
// ── Live staff finance ────────────────────────────────────────────────────────
// Returns staff_payroll_profiles (non-instructor or mixed/fixed roles) with
// their adjustments in the date range.
...
export async function getLiveStaffFinance(
  ...
  // 1. Load staff profiles
  const { data: profileData } = await db
    .from('staff_payroll_profiles')
    .select(`
      id, user_id, branch_id, role, payroll_type, basic_salary, session_rate,
      payment_method, payment_reference, is_payroll_enabled, notes,
  ...
  return profiles.map(p => {
    ...
    return {
      ...
      is_payroll_enabled: p.is_payroll_enabled,
      notes:              p.notes ?? null,
      adjustments,
      ...totals,
      net_amount: basic_salary + totals.adj_net,
...
export async function searchUsersForStaff(
  ...
  // Search profiles by first/last name
...
// ── Dashboard live payroll summary (replaces getDashboardPayrollSummary) ───────
